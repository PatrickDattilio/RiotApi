...
public class ItemJsonDeserializer implements JsonDeserializer<ItemsJsonResponse> {
    ...
    public ItemsJsonResponse deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
        ...
        for (Map.Entry<String, JsonElement> entry : set) {
            ...
            Item nextItem = jsonDeserializationContext.deserialize(entry.getValue(), new TypeToken<Item>() {
            }.getType());
        ...
        ArrayList<ItemGroup> groups = jsonDeserializationContext.deserialize(obj.get("groups"), new TypeToken<ArrayList<ItemGroup>>() {
        }.getType());
        ...
        ArrayList<ItemTree> treeObj = jsonDeserializationContext.deserialize(obj.get("tree"), new TypeToken<ArrayList<ItemTree>>() {
        }.getType());
